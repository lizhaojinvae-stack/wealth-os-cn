...
export function ensureAuthSchema(): Promise<void> {
  if (!authSchemaPromise) {
    authSchemaPromise = (async () => {
      ...
      await db.batch([
        db.prepare(
          "CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, email TEXT NOT NULL UNIQUE COLLATE NOCASE, display_name TEXT NOT NULL, password_hash TEXT NOT NULL, password_salt TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)",
        ),
        db.prepare(
        ...
        db.prepare(
          "CREATE TABLE IF NOT EXISTS user_data (user_id TEXT PRIMARY KEY, payload TEXT NOT NULL DEFAULT '{}', revision INTEGER NOT NULL DEFAULT 1, updated_at TEXT NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)",
        ),
        db.prepare(
          "CREATE TABLE IF NOT EXISTS ai_provider_keys (user_id TEXT NOT NULL, provider TEXT NOT NULL, encrypted_key TEXT NOT NULL, iv TEXT NOT NULL, key_hint TEXT NOT NULL, updated_at TEXT NOT NULL, PRIMARY KEY (user_id, provider), FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE)",
        ),
      ]);
    })().catch((error) => {
...
export async function verifyPassword(
  ...
  for (let index = 0; index < hash.length; index += 1)
    diff |= hash.charCodeAt(index) ^ expected.charCodeAt(index);
  return diff === 0;
}
...
export function readSessionCookie(request: Request) {
  const value = request.headers
    ...
    ?.split(";")
    .map((part) => part.trim())
    .find((part) => part.startsWith("wealth_session="));
...
export const sessionCookie = (token: string, maxAge = 30 * 86400) =>
  `wealth_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${maxAge}`;
export const publicUser = (row: UserRow): AuthUser => ({
  id: row.id,
  email: row.email,
