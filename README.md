...
- 策略回测：真实前复权日线、下一交易日成交、手续费与滑点、净值对照、最大回撤和逐笔审计
...
| 策略回测 | 东方财富前复权日线，腾讯证券备用 | 用户主动运行，单次最多约600个交易日 |
...
回测 v1 使用固定透明规则：收盘突破此前20日最高价、MA20高于MA60且量比不低于1.2后，于下一交易日开盘买入；收盘跌破MA20、相对成本下跌8%或持有满20个交易日后，于下一交易日开盘卖出。回测按A股100股整数手计入双边手续费和滑点，不使用未来数据。当前版本尚未逐日还原涨跌停无法成交、停牌、印花税历史变化和大额交易容量冲击，因此结果只用于研究规则，不代表真实可复制收益。
